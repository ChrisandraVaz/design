...
export default function TraceCaseNavigation() {
 ...
 return <>
  <nav className="trace-case-top" aria-label="Case study navigation"><Link href="/" className="trace-case-back back-home"><span aria-hidden="true">‹</span>Back to Home</Link><div className="trace-case-tabs">{[['trace-overview','Overview'],['intent','Problem'],['motion','Solution'],['takeaways','Takeaways']].map(([id,label])=><a key={id} href={`#${id}`} aria-current={tab===id?'location':undefined}>{label}</a>)}</div></nav>
