...
export default function AgentMotionPreview({paused = false, showCompletedQuery = false, containFrame = false}: {paused?: boolean; showCompletedQuery?: boolean; containFrame?: boolean}) {
  const root = useRef<HTMLDivElement>(null);
  const pauseRef = useRef(paused);
  const syncPlayback = useRef<() => void>(() => {});
  const [step, setStep] = useState(13);
  const [shot, setShot] = useState(showCompletedQuery ? "overview" : "thinking");
  const [queryStage, setQueryStage] = useState<QueryStage>(showCompletedQuery ? "complete" : "thinking");
  const [queryElapsed, setQueryElapsed] = useState(0);
  const [answerVisible, setAnswerVisible] = useState(showCompletedQuery);
  ...
  useEffect(() => {
    ...
    const cameraFrames = (): Keyframe[] => {
      const openingView = containFrame ? "translate(0,0) scale(.84)" : opening;
      ...
      const closeUp = containFrame ? "translate(0,0) scale(.88)" : `translate(${closeUpX}px,${closeUpY}px) scale(${closeUpScale})`;
      ...
      const resultView = containFrame ? openingView : `translate(0,${resultOffsetY}px) scale(${resultScale})`;
      return [
        {transform: openingView, opacity: 1, offset: 0},
        {transform: openingView, offset: (1800 + queryIntro) / duration, easing: ease},
        ...
        {transform: openingView, opacity: 1, offset: (17800 + menuTimingOffset) / duration},
        {transform: openingView, opacity: 1, offset: 1},
      ];
    };
    ...
    const conversationMotion = body.animate(showCompletedQuery ? [{opacity: 1}, {opacity: 1}] : [
      {opacity: 1, offset: 0},
    ...
    let lastQuery: QueryStage = showCompletedQuery ? "complete" : "thinking";
    let lastElapsed = 0;
    let lastAnswer = showCompletedQuery;
    const tick = () => {
      const time = Number(clock.currentTime ?? 0) % duration;
      const handoffTime = time - queryIntro;
      const stage: QueryStage = showCompletedQuery ? "complete" : time < 1000 || time >= resetAt ? "thinking" : time < 3200 ? "querying" : "complete";
      const elapsed = stage === "thinking" ? 0 : Math.min(2.2, Math.floor((time - 1000) / 100) / 10);
      const answer = showCompletedQuery || (time >= 3800 && time < resetAt);
      ...
      body.scrollTop = containFrame ? 0 : Math.max(0, body.scrollHeight - body.clientHeight) * scrollEase;
  ...
  }, [showCompletedQuery, containFrame]);

  return <div ref={root} className={`agent-film${containFrame ? " is-contained" : ""}`} data-shot={shot} data-playing={playing} data-paused={paused} aria-hidden="true">
