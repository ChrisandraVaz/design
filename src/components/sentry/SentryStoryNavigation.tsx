...
export default function SentryStoryNavigation({kind}:{kind:string}) {
 ...
 return <><nav className="trace-case-top" aria-label="Case study navigation"><Link href="/" className="trace-case-back"><span aria-hidden="true">‹</span>Back to Home</Link><div className="trace-case-tabs">{[['overview','Overview'],['the-problem','Problem'],['solution','Solution'],['takeaways','Takeaways']].map(([id,label])=><a key={id} href={`#${id}`} aria-current={tab===id?'location':undefined}>{label}</a>)}</div></nav><nav className="trace-case-tree" aria-label={`${queue?'Message Queuing':split?'Split Panel':'Relative Time'} chapters`}>{groups.map(g=><div className="trace-case-tree-group" key={g.id}><a href={`#${g.id}`} aria-current={active===g.id?'location':undefined}>{g.label}</a>{g.children&&<div className="trace-case-branches">{g.children.map(([id,label])=><a key={id} href={`#${id}`}>{label}</a>)}</div>}</div>)}</nav></>;
}
